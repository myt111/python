...
def postgre_execute_hw():
    ...
    select
            alarm_name ,
            alarm_level,
            to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) ) + 6 || ' day' ) :: INTERVAL ) :: DATE ,'YYYY-MM-DD') as start_time ,
...
# where
# enodebid != '		-'
# and 上周 to_char(cast(start_time as timestamp), 'yyyy-mm-dd') <= to_char(
#     (DATE(now()) - (EXTRACT(dow FROM DATE ( now())) - 1 | | ' day' ):: INTERVAL ):: DATE - 1, 'YYYY-MM-DD') and
# to_char(cast(start_time as timestamp), 'yyyy-mm-dd') >= to_char(
#     (DATE(now()) - (EXTRACT(dow FROM DATE ( now())) - 1 | | ' day' ):: INTERVAL ):: DATE - 7, 'YYYY-MM-DD')

# 本周 and to_char(cast(start_time as timestamp) , 'yyyy-mm-dd') >= to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) ) - 1 || ' day' ) :: INTERVAL ) :: DATE ,'YYYY-MM-DD')
...
def postgre_execute_zx():
    conn = psycopg2.connect(database="expert-system", user="expert-system", password="YJY_exp#exp502",
                            host="10.1.77.51", port="5432")
    print("ZX Postgres Connection success")
    ...
    """
    # and to_char(cast(start_time as timestamp) , 'yyyy-mm-dd') <= to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) ) - 1 || ' day' ) :: INTERVAL ) :: DATE-1 ,'YYYY-MM-DD') and
    # to_char(cast(start_time as timestamp) , 'yyyy-mm-dd')  >= to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) ) - 1 || ' day' ) :: INTERVAL ) :: DATE-7 ,'YYYY-MM-DD')
# and to_char(cast(start_time as timestamp) , 'yyyy-mm-dd') >= to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) ) - 1 || ' day' ) :: INTERVAL ) :: DATE ,'YYYY-MM-DD')
    cur.execute(sql_alarm)
    alarm_enb = cur.fetchall()
...
def postgre_upload_hw(data):
    ...
    cur = conn.cursor()
    cur.copy_from(StringIO(output1), "alarm_hz_week_1128")
    conn.commit()
    cur.close()
    conn.close()
    print("alarm_hw_week sync: success")
