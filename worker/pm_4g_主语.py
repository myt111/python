...
def get_time():
    ...
    previous_time = now_time + dt.timedelta(hours=-11)
    previous_day = now_time + dt.timedelta(days=-3)
...
if __name__ == '__main__':
	...
	cast(format_datetime(cast(start_time as timestamp),'yyyyMMdd') as int) as daytime,
    cast(format_datetime(cast(start_time as timestamp),'yyyyMMddHH') as int) as hourtime
    from ( select *
from hive.cnio.pm_4g where cast(pm_date as varchar ) ='%s'  ) x where substr(x.start_time,12,2)= '%s' limit 10
 ''' % (prev_date,prev_hour)
