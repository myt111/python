...
def get_date():
    ...
    yestoday = today + dt.timedelta(days=(-3))
    yestoday_time = yestoday.strftime('%Y-%m-%d')
   # yestoday_time_extra = yestoday.strftime('%Y%m%d')
    previous_time = yestoday.strftime('%H')
    return previous_time

def trino_execute(yestoday_time):
...
where   concat_ws('-',substring(pm_date,1,4),substring(pm_date,5,2),substring(pm_date,7,2)) 
= cast(current_date- interval '3' day  as varchar ) 
and format_datetime(cast(start_time as timestamp),'yyyy-MM-dd-HH') =  concat_ws('-',cast(current_date - interval '3' day as varchar),cast(%s as varchar)) limit 10 """ % yestoday_time
    cur.execute(sql_alarm)
    pm_siteinfo = cur.fetchall()

    pm_pd = []
    siteinfo_pd = []
    if len(pm_siteinfo) != 0:
        pm_pd = pd.DataFrame(pm_siteinfo)
        des = cur.description
        colname = []
        for item in des:
            colname.append(item[0])
        pm_pd.columns = colname

        # conn1 = psycopg2.connect(database="expert-system", user="expert-system", password="YJY_exp#exp502",
        #                         host="10.1.77.51", port="5432")
...
if __name__ == '__main__':
    yestoday = get_date()
    pm, siteinfo = trino_execute(yestoday)
    data = postgre_pm_hour(pm, siteinfo)
    print("ok")
    postgre_upload(data)
