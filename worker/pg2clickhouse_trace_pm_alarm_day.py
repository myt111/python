...
if __name__ == '__main__':
    ...
    columns_list_str = ",".join(columns_list['column'].values) # cellname String,cellindex Int64,pci Int32

    df['target'] = df.iloc[:, 2] + ' ' + df.iloc[:, 3]
    columns_type_list_4trino = ','.join(df['target'].values)

    print(columns_list['type'] )
   ...
   # client = Client(host='10.1.77.51', port=9008, user='default', database='default', password='YJY_cu#unicom502')
    ...
    """.format(table_name, ','.join(columns_list['column_name'].values), ','.join(columns_list['column_name'].values),
               db_name, schema_name,
               table_name)
