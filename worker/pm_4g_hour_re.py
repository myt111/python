...
def get_time():
    ...
    previous_time = now_time + dt.timedelta(hours=-10)
...
def postgre_expert(data):
    ...
    # data.to_sql("pm_4g_zdy_hour",engine,schema="searching",  index=False, if_exists='append')
    ...
    data["daytime"]=data["daytime"].astype(pd.Int64Dtype())
    data["hourtime"]=data["daytime"].astype(pd.Int64Dtype())
    data = data.replace('\x00', value='',inplace=False)

    result = data
    output = StringIO()
    result.to_csv(output, sep='\t', index=False, header=False)
    output1 = output.getvalue()
    conn = psycopg2.connect(host="133.160.191.111", user ="expert-system", password = "YJY_exp#exp502", database = "expert-system",port="5432",options="-c search_path=searching" )
    cur = conn.cursor()
    cur.copy_from(StringIO(output1), "pm_4g_zdy_hour")
    print("PM小时粒度数据入库完成")
    conn.commit()
    cur.close()
    conn.close()
...
from hive.cnio.pm_4g  where  pm_date=  to_char(current_date - interval '1' day ,'yyyymmdd')  limit 10'''
