...
def get_date():
    ...
    last_time = now_time + dt.timedelta(hours=(-12))
...
def postgre_execute_hw(yestoday):
    ...
    select
        ...
        where
            ...
            and to_char(cast(start_time as timestamp) , 'yyyy-mm-dd-hh24')>= '%s'
...
def postgre_upload_hw(data):
    ...
    cur = conn.cursor()
    cur.copy_from(StringIO(output1), "alarm_hz_hour_1128",null='')
    conn.commit()
    cur.close()
    conn.close()
...
if __name__ == '__main__':
   # hour = get_date()
    ...
    alarm, siteinfo = postgre_execute_hw(hour)
    if len(alarm)!=0:
        alarm_hw =  postgre_alarm_hw_day(alarm, siteinfo)
        print("ok")
        postgre_upload_hw(alarm_hw)

    alarm_zx, siteinfo = postgre_execute_zx(hour)
    if len(alarm)!=0:
       
        alarm_zx = postgre_alarm_zx_day(alarm_zx, siteinfo)
        print("ok")
