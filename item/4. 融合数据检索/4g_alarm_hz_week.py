...
def get_date():
    ...
    yestoday = today + dt.timedelta(days=(-8))
...
def postgre_execute_hw():
    ...
    select
            alarm_name ,
            alarm_level,
            to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) )  || ' day' ) :: INTERVAL ) :: DATE  ,'YYYY-MM-DD') as start_time ,
            cast(enodebid as int) as enodebid,
            locate_info
        from
            cnio.lte_alarm_detail_hw ahh
        where
            enodebid != '		-'  and  enodebid != '264008.0'   and  enodebid != '268401.0'   and enodebid != '262219.0' and enodebid != '263846.0'  and  start_time != '发生时间 (NT)'
...
def postgre_upload_hw(data):
    ...
    data.to_sql("alarm_hz_week", engine, schema="searching", index=False, if_exists='append', method='multi')
    #output = StringIO()
