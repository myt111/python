...
def get_date():
    ...
    last_time = now_time + dt.timedelta(hours=(-9))
    last_time = now_time + dt.timedelta(days=(-8))
...
def postgre_execute_hw():
    ...
    select
            alarm_name ,
            alarm_level,
            to_char(cast(start_time as timestamp) , 'yyyy-mm-dd-hh24') as start_time ,
            cast(enodebid as float8) as enodebid,
            locate_info
        from
            cnio.lte_alarm_detail_hw ahh
        where
            enodebid != '		-' and  start_time != '发生时间 (NT)'
      and   to_char(cast(start_time as timestamp) , 'yyyy-mm-dd') >=  to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) )  || ' day' ) :: INTERVAL ) :: DATE -6  ,'YYYY-MM-DD')
            and  to_char(cast(start_time as timestamp) , 'yyyy-mm-dd') <=  to_char(( DATE ( now() ) - ( EXTRACT ( dow FROM DATE ( now() ) )  || ' day' ) :: INTERVAL ) :: DATE  ,'YYYY-MM-DD')
...
def postgre_execute_zx():
    ...
    select
            ...
            addition_text
        from
            "expert-system".cnio.lte_alarm_detail_zte
        where
...
def postgre_upload_hw(data):
    ...
    data.to_sql("alarm_hz_hour", engine, schema="searching", index=False, if_exists='append', method='multi')
    #output = StringIO()
   ...
   # cur = conn.cursor()
   # cur.copy_from(StringIO(output1), 'alarm_hz_hour',null='')
   # conn.commit()
   # cur.close()
   # conn.close()
    print("alarm_hw_hour sync: success")
...
def postgre_upload_zx(data):
   ...
   # cur.copy_from(StringIO(output1), 'alarm_hz_hour',null='')
    ...
    #cur.close()
    #conn.close()
    print("alarm_zx_hour sync: success")

if __name__ == '__main__':
    yestoday = get_date()
   # yestoday = '2022-11-14-23'
