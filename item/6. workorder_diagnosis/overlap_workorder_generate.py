...
def dbscan_cluster(data):
    ...
    ## 密度聚类
    db = DBSCAN(eps=epsilon, min_samples=10, algorithm='ball_tree', metric='haversine').fit(np.radians(df.astype(float)))
    ## 聚类标签
...
def data_input_postgres(data_cluster,date_list):
    ...
    data["root_cause"] = "覆盖不合理，导致缺少主服务小区"
    data["order_status"] = ""
    ...
    print("overlap workorder is inserted into fault_workorder ")
...
if __name__ == '__main__':
    ...
    trace_overlap_grid=postgre_trace(date_list)

    if  len(trace_overlap_grid)==0:
        print("数据为空，工单生成失败")
    else:
        data_cluster  = dbscan_cluster(trace_overlap_grid)
        #data_cluster.to_csv('overlap_cluster.csv')
