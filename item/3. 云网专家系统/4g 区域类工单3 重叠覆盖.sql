...
insert into cnio.overlap_coverage_area_cell
...
from (
       ...
       from (select a.area_id, a.lon, a.lat
             ...
             where fault_type = '重叠覆盖'
                   group by area_id) a
            ) c
                left join
            cnio.trace_fault_grid  b on b.area_id = c.area_id where fault_type = '重叠覆盖'
   ) d
...
insert into cnio.trace_mr_grid_cell
select 	gridx,
	gridy ,
	gridlat ,
	gridlon ,
	...
	direct,
	trace_date
from cnio.trace_mr_grid_cell_all
where trace_date >= cast(to_char(now()::timestamp  + '-2 day +8 hour', 'yyyy-mm-dd') as date);
...
select 	gridx,
	...
	dist,
	direct,
	cast(to_char(now()::timestamp  + '-2 day +8 hour', 'yyyy-mm-dd') as date) trace_date
from cnio.task_43_lte_mr_grid_cell;


             
      
insert into cnio.overlap_info_area_cell
select e.area_id,
       e.lon1 as lon,
       e.lat1 as lat,
       e.gridx,
               ...
               from (
                        ...
                        from (
                                 ...
                                 from (select a.lon,
                                       ...
                                       where fault_type = '重叠覆盖' and fault_date  = cast(to_char(now()::timestamp  + '-2 day +8 hour', 'yyyy-mm-dd') as date)
                                      ) c1,
                                      (
...
insert into cnio.overlap_cell_weakcoverageall 
         ...
         from (
                  ...
                  from (
                           ...
                           from (
                                    ...
                                    from cnio.overlap_info_area_cell
                                    group by areaid
                                ) t1
                           group by areaid) t2
                           left join
                  ...
                  left join(
             select areaid, count(eci) as ecicnt from cnio.overlap_info_area_cell group by areaid
         ) t5
                  ...
                  from (
                           ...
                           from (
                                    ...
                                    from (
                                             ...
                                             from cnio.overlap_coverage_area_cell) tt1
                                ) t1
                                    left join
                       ...
                       (
                           ...
                           from (
                                    ...
                                    union all
                                    select cast(area_id as bigint), cast(eci as integer)
                                    from cnio.overlap_coverage_area_cell) t1
...
having count(tt2.eci)>=3
     ...
     on cast(m1.areaid as bigint) = cast(m6.areaid as bigint)
         left join
     (
         select area_id, lon_area, lat_area, cnt from cnio.overlap_coverage_area_cell group by area_id, lon_area, lat_area, cnt
     ) m7
...
insert into cnio.overlap_all
select m5.*,
        case
           when cast(alarm_cell_num as text) is not null then '维护'
           else
               (case
                    when cast(alarm_cell_num as text) is null and cast(azimuthcheck_cell_num as numeric) !=0 and cast(locationerr_cell_num as numeric) !=0 and cast(overshooting_cell_num as numeric) !=0 then '优化'
                    else
                   (
                       case when cast(alarm_cell_num as text) is null and cast(azimuthcheck_cell_num as numeric) =0 and cast(locationerr_cell_num as numeric) =0 and cast(overshooting_cell_num as numeric) =0 then '专家诊断' end
                       )
                   end) end as order_sorting,

                                 case
                                     when cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then '1. 【小区/基站告警】 2. 【天线方位角异常】 3.【越区覆盖】'
                                           when cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '1. 【小区/基站告警】 2. 【天线方位角异常】 3.【越区覆盖】'
                                     ...
                                     cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                     cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then '1. 【小区/基站告警】 2. 【天线方位角异常】 4. 【基站位置错误】'
                                     when cast(m5.alarm_cell_num as text) is not null and 
                                     cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0
                                          and cast(m5.locationerr_cell_ratio as numeric) > 0 then '1. 【小区/基站告警】 3. 【越区覆盖】 4. 【基站位置错误】'
                                     when cast(m5.alarm_cell_num as text) is null and 
                                     cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then '2. 【天线方位角异常】  3. 【越区覆盖】 4. 【基站位置错误】'
                                     when  
                                    	 cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '1. 【小区/基站告警】 2. 【天线方位角异常】'
                                     when  
                                    	 cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2   and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '1. 【小区/基站告警】 3. 【越区覆盖】'
                                          when  
                                    	 cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 then '1. 【小区/基站告警】 4. 【基站位置错误】'
                                          when  
                                    	  cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and  cast(m5.alarm_cell_num as text) is  null and 
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '2. 【小区/基站告警】 3. 【天线方位角异常】'
                                          when  
                                    	 cast(m5.locationerr_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                          cast(m5.alarm_cell_num as text) is  null and  
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 then '2. 【小区/基站告警】 4. 【天线方位角异常】'
                                          when  
                                    	  cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 and cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2  then '3. 【越区覆盖】 4. 【基站位置错误】'
                                       when    
                                    cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then '1. 【小区/基站告警】'
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then '2. 【天线方位角异常】'
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then '3. 【越区覆盖】'
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0  then '4. 【基站位置错误】'
                                           end    as root_cause,
 ...
 case
                                     when cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then '1. 恢复小区影响业务故障 2. 天线方位角/波瓣角调整 3.天馈调整'
                                           when cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '1. 恢复小区影响业务故障 2. 天线方位角/波瓣角调整 3.天馈调整'
                                     ...
                                     cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                     cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then '1. 恢复小区影响业务故障 2. 天线方位角/波瓣角调整 4. 核实修改工参数据'    
                                     when cast(m5.alarm_cell_num as text) is not null and 
                                     cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0
                                          and cast(m5.locationerr_cell_ratio as numeric) > 0 then '1. 恢复小区影响业务故障 3. 天馈调整 4. 核实修改工参数据'
                                     when cast(m5.alarm_cell_num as text) is null and 
                                     cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then '2. 天线方位角/波瓣角调整  3. 天馈调整 4. 核实修改工参数据'
                                     when  
                                         cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '1. 恢复小区影响业务故障 2. 天线方位角/波瓣角调整'
                                     when  
                                         cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2   and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '1. 恢复小区影响业务故障 3. 天馈调整'
                                          when  
                                         cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 then '1. 恢复小区影响业务故障 4. 核实修改工参数据'
                                          when  
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and  cast(m5.alarm_cell_num as text) is  null and 
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then '2. 恢复小区影响业务故障 3. 天线方位角/波瓣角调整'
                                          when  
                                         cast(m5.locationerr_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                          cast(m5.alarm_cell_num as text) is  null and  
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 then '2. 恢复小区影响业务故障 4. 天线方位角/波瓣角调整'
                                          when  
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 and cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2  then '3. 天馈调整 4. 核实修改工参数据'
                                       when    
                                    cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then '1. 恢复小区影响业务故障'
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then '2. 天线方位角/波瓣角调整'
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then '3. 天馈调整'
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0  then '4. 核实修改工参数据'
 ...
 case
                                     when cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then concat('1. 区域范围内',m5.alarm_cell_num,'个服务小区/基站出现告警','2. 服务小区平均距离',m5.average_dist,'米，服务小区平均RSRP=,',m5.servingrsrp,'dBm,最强小区的RSRP=',m5.bestrsrp,'dBm,服务且非最强的栅格数为',m5.servingnobest_grid_num,'个','4. 基站位置错误，核实工参数据')
                                           when cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then concat('1. 区域范围内',m5.alarm_cell_num,'个服务小区/基站出现告警','2. 服务小区平均距离',m5.average_dist,'米，服务小区平均RSRP=,',m5.servingrsrp,'dBm,最强小区的RSRP=',m5.bestrsrp,'dBm,服务且非最强的栅格数为',m5.servingnobest_grid_num,'个','4. 基站位置错误，核实工参数据')
                                     ...
                                     cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0
                                          and cast(m5.locationerr_cell_ratio as numeric) > 0 then concat('1. 区域范围内',m5.alarm_cell_num,'个服务小区/基站出现告警 3. 基站位置错误，核实工参数据 4. 基站位置错误，核实工参数据')
                                     when cast(m5.alarm_cell_num as text) is null and 
                                     cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 then concat('2. 服务小区平均距离',m5.average_dist,'米，服务小区平均RSRP=,',m5.servingrsrp,'dBm,最强小区的RSRP=',m5.bestrsrp,'dBm,服务且非最强的栅格数为',m5.servingnobest_grid_num,'个 3. 基站位置错误，核实工参数据 4. 基站位置错误，核实工参数据')
                                     when  
                                         cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then concat('1. 区域范围内',m5.alarm_cell_num,'个服务小区/基站出现告警','2. 服务小区平均距离',m5.average_dist,'米，服务小区平均RSRP=,',m5.servingrsrp,'dBm,最强小区的RSRP=',m5.bestrsrp,'dBm,服务且非最强的栅格数为',m5.servingnobest_grid_num,'个')
                                     when  
                                         cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2   and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then concat('1. 区域范围内',m5.alarm_cell_num,'个服务小区/基站出现告警 3. 基站位置错误，核实工参数据')
                                          when  
                                         cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 then concat('1. 区域范围内',m5.alarm_cell_num,'个服务小区/基站出现告警 4. 基站位置错误，核实工参数据')
                                          when  
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and  cast(m5.alarm_cell_num as text) is  null and 
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0 then concat('2. 服务小区平均距离',m5.average_dist,'米，服务小区平均RSRP=,',m5.servingrsrp,'dBm,最强小区的RSRP=',m5.bestrsrp,'dBm,服务且非最强的栅格数为',m5.servingnobest_grid_num,'个','基站位置错误，核实工参数据')
                                          when  
                                         cast(m5.locationerr_cell_ratio as numeric) > 0 and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and 
                                          cast(m5.alarm_cell_num as text) is  null and  
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 then concat('2. 服务小区平均距离',m5.average_dist,'米，服务小区平均RSRP=,',m5.servingrsrp,'dBm,最强小区的RSRP=',m5.bestrsrp,'dBm,服务且非最强的栅格数为',m5.servingnobest_grid_num,'个','4. 调整越区小区的天馈问题')
                                          when  
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0 and cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2  then '3. 基站位置错误，核实工参数据 4. 基站位置错误，核实工参数据'
                                       when    
                                    cast(m5.alarm_cell_num as text) is not null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then concat('1. 区域范围内',m5.alarm_cell_num,'个服务小区/基站出现告警')
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) > 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then concat('2. 服务小区平均距离',m5.average_dist,'米，服务小区平均RSRP=,',m5.servingrsrp,'dBm,最强小区的RSRP=',m5.bestrsrp,'dBm,服务且非最强的栅格数为',m5.servingnobest_grid_num,'个')
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) > 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) <= 0  then '3. 基站位置错误，核实工参数据'
                                                 when    
                                    cast(m5.alarm_cell_num as text) is  null and
                                          cast(m5.azimuthcheck_cell_ratio as numeric) <= 0.2 and
                                          cast(m5.overshooting_cell_ratio as numeric) <= 0 and
                                          cast(m5.locationerr_cell_ratio as numeric) > 0  then '4. 基站位置错误，核实工参数据'
                                           end   
...
from (
         ...
         from cnio.overlap_cell_weakcoverageall) m1
                  left join (
             select area_id, avg(cast(dist as double precision)) as average_dist
             from cnio.overlap_coverage_area_cell
             group by area_id
         ) m2
                            on cast(m1.areaid as integer) = cast(m2.area_id as integer)
                  left join (
             select f.area_id, avg(f.servingrsrp) as servingrsrp, avg(f.bestrsrp) as bestrsrp
             from (select *
                   from (select b.fault_id,
                                b.gridx,
                                b.gridy,
                                b.lon,
                                b.lat,
                         ...
                         from cnio.trace_fault_grid b
                              ...
                              (
                                  ...
                                  from cnio.trace_fault_grid
                                  where fault_type = '重叠覆盖'
                                  group by area_id) a on
                                  a.area_id = b.area_id) c
                            left join
                        (select *
                         from cnio.trace_mr_grid tmg
                         where trace_date >= cast(to_char(now()::timestamp  + '-2 day +8 hour', 'yyyy-mm-dd') as date)) e
                        on c.gridx = e.gridx and c.gridy = e.gridy
                   where fault_type = '重叠覆盖') f
             group by f.area_id
         ) m3
                            on cast(m1.areaid as integer) = cast(m3.area_id as integer)
                  left join (
             select k.area_id      as area_id,
                    max(k.sumrsrp) as servingnobest_grid_num
             from (
                      select c.area_id,
                             e.trace_date,
                             sum(case when e.servingrsrp - e.bestrsrp <= 0 then 1 else 0 end) sumrsrp
                      from (
                               select b.fault_id,
                                      b.gridx,
                                      b.gridy,
                           ...
                           on c.gridx = e.gridx and c.gridy = e.gridy
                      where fault_type = '重叠覆盖'
                      group by c.area_id, e.trace_date) k
             group by k.area_id
         ) m4
                           on cast(m1.areaid as integer) = cast(m4.area_id as integer)) m5;
