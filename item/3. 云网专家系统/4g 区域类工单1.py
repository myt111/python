...
def get_overlap_grid(date_list,expert_db_account):
    ...
    # 执行查询
    sql_trace_sk_task = """
    select m.gridx,m.gridy,m.lon,m.lat,(overlap3sample*1.0/m.servtotalnum) as overlap  from (
...
servrsrpdistr7+servrsrpdistr8+servrsrpdistr9) as servtotalnum,sum(tmg.overlap3sample) as overlap3sample from cnio.trace_mr_grid tmg  
where trace_date =%s or trace_date =%s or trace_date =%s or trace_date =%s 
 or trace_date =%s or trace_date =%s or trace_date =%s  group by gridx,gridy,lon,lat ) m  where m.servtotalnum>100 and (overlap3sample*1.0/m.servtotalnum)>0.3
    """
    # 查询taskid
    cur.execute(sql_trace_sk_task, date_list)
    trace_overlap_grid = cur.fetchall()
    if len(trace_overlap_grid) == 0:
        pass
    else:
        trace_overlap_grid = pd.DataFrame(trace_overlap_grid)
        # 表描述
        des = cur.description
        # 获取表头
        colname = []
        ...
        trace_overlap_grid.columns = colname

    cur.close()
    conn.close()
    print("Postgres Connection release")
    print(trace_overlap_grid.index)
    return trace_overlap_grid
...
def data_upload_postgres(data_cluster, date_list,expert_db_account,fault_workorder_table,fault_grid_table):
    ...
    print("重叠覆盖工单同步至fault_workorder表单")
    ...
    trace_fault_grid["fault_type"] = "重叠覆盖"
    trace_fault_grid["fault_date"] = max(date_list)

    trace_fault_grid = trace_fault_grid[
        ["fault_id", "area_id", "gridx", "gridy", "lon", "lat", "fault_type", "fault_date"]]
    data_upload_stringio(trace_fault_grid, fault_grid_table, expert_db_account)

    print("重叠覆盖栅格同步至trace_fault_grid表单")
...
if __name__ == '__main__':
    ...
    trace_overlap_grid = get_overlap_grid(date_list,expert_db_account)

    if len(trace_overlap_grid) == 0:
        print("无重叠覆盖栅格，工单生成失败")
    else:
        data_cluster = dbscan_cluster(trace_overlap_grid)
        data_upload_postgres(data_cluster, date_list,expert_db_account,fault_workorder_table,fault_grid_table)
