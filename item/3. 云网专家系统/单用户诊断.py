...
def get_date(day_num):
    ...
    # 北京部署需计算话单时间差
    delta_ms = (dt.datetime(yestoday.year, yestoday.month, yestoday.day) - dt.datetime(2022, 6,
                                                                                             15)).days * 86400 * 1000
    return yestoday_date,yestoday_date_, year,delta_ms
...
def get_analysis_data(yestoday, yestoday_date_,year,expert_db_account):
    ...
    sql_pm = """
        select
        eci,
        ul_busy_prb_use_ratio as busy_ul_prb_use_ratio,
        dl_busy_prb_use_ratio as busy_dl_prb_use_ratio,
        idletime_prb_noise_power_dbm as idle_prb_noise_poser
    from
        cnio.pm_4g_workorder pgzd
    where
        trace_date = '%s' 
    """ % yestoday_date_
    cur.execute(sql_pm)
    ...
    if len(siteinfo_data) != 0:
        ...
        ## 异常数据处理
        siteinfo_data_pd["cell_name"]=siteinfo_data_pd["cellname"].apply(lambda x:x.replace("\n",""))
        siteinfo_data_pd["azimuth_v"]=siteinfo_data_pd["azimuth"].apply(lambda x:(x.split("/"))[0] if "/" in x else x )
        siteinfo_data_pd = siteinfo_data_pd.loc[:, ~siteinfo_data_pd.columns.isin(["cellname","azimuth"])]
        siteinfo_data_pd.rename(columns={"cell_name":"cellname","azimuth_v":"azimuth"},inplace=True)
        siteinfo_data_pd=siteinfo_data_pd[["eci","indoor","celllon","celllat","height","azimuth","tilt","cellname","channel"]]
...
def get_call_recorder(yestoday, trace_db_account):
    ...
    ## 如果taskid不为空，
    if len(taskid) == 0:
        print("未查询到trace解析对应的任务编号taskid")
        data=[]
    else:
        ...
        data = cur.fetchall()
        if len(data) == 0:
            print("异常话单查询返回空值，需确认lte_conn表单是否为空表")
        else:
...
##  话单关联指标
def form_association(trace_data, pm_data, alarm_data, plansite_data, top1_site_data, siteinfo_data):
    #trace_data.to_csv("test.csv",index=False)
    # 重叠覆盖率计算
...
def diagnosis_algrithom(single_user_recorder):
    ...
    for i in single_user_recorder.index:
        ...
        if not pd.isnull(src_ul_prb_total) and src_ul_prb_total > 0:
            src_ul_prb_ratio = round(src_ul_prb / src_ul_prb_total, 2) * 100
            if src_ul_prb_ratio > 100:
                if not pd.isnull(src_busy_ul_prb_use_ratio):
        ...
        if not pd.isnull(src_dl_prb_total) and src_dl_prb_total > 0:
            ...
            if src_dl_prb_ratio > 100 :
                if not pd.isnull(src_busy_dl_prb_use_ratio):
        ...
        if connstatus_list[str(connstatus)] == "ConnFail":
            if connesttype_list[str(connesttype)] == "RRCSetup":
                ...
                if servicetype_list[str(servicetype)] == "Voice" or servicetype_list[str(servicetype)] == "IMSData":
                    description = "RRC初始建立失败，语音业务未接通，携带原因值为" + endcause_list[str(endcause)]
                else:
                    description = "RRC初始建立失败，数据业务建立失败，携带原因值为" \
                                  + endcause_list[str(endcause)]
            elif connesttype_list[str(connesttype)] == "RRCReest":
                if servicetype_list[str(servicetype)] == "Voice" or servicetype_list[str(servicetype)] == "IMSData":
                    description = "RRC重建失败，语音业务掉话，携带原因值为" + endcause_list[str(endcause)]
                else:
                    description = "RRC重建失败，数据业务掉线，携带原因值为" \
                                  + endcause_list[str(endcause)]
            else:
                if servicetype_list[str(servicetype)] == "Voice" or servicetype_list[str(servicetype)] == "IMSData":
                    description = "UE切入失败，携带原因值为" + endcause_list[str(endcause)]
                else:
                    description = "UE切入失败，携带原因值为" \
                                  + endcause_list[str(endcause)]
            ##  根因诊断
            if not pd.isnull(src_alarm_num) and src_alarm_num > 0:
                ...
                if src_indoor == 1:
                    classify = "优化"
                    root_cause = "接入小区为室分小区，接入位置覆盖差，接入RSRP" + str(servrsrp) + "dBm"
                    action = "建议排查接入小区网络覆盖，提升小区覆盖能力"
                else:
                    if not pd.isnull(src_plansite_flag) and src_plansite_flag == 1:
                        ...
                        classify = "优化"
                        root_cause = "服务小区近距离弱覆盖，疑似建筑物阻挡"
                        action = "建议排查无线环境，若存在阻挡，调整基站位置或天线挂高"
                    elif to_src_cell_distance > 300 and to_src_cell_distance < 5000:
                        classify = "优化"
                        root_cause = "用户处于服务小区覆盖远点"
                        action = "建议抬高下倾角或者增大小区发射功率，提升小区覆盖能力"
                    else:
                        classify = "优化"
                        root_cause = "用户接入位置覆盖差,信息不足无法进一步定位"
                        action = "建议路测排查弱覆盖区域并进行整改，提升小区覆盖能力"
                ...
                classify = "优化"
                root_cause = "接入小区越区覆盖，用户到服务小区距离" + str(to_src_cell_distance) + "米"
                action = "建议压低接入小区天线下倾角或者降低站高"
                ...
                classify = "维护"
                root_cause = "接入小区存在上行干扰,上行sinr值为" + str(src_ul_sinr_avg) + "dB"
                action = "建议排查上行干扰"
            elif (not pd.isnull(overlap) and overlap > 0.3):
                classify = "优化"
                root_cause = "接入小区下行重叠覆盖率高，为" + str(round(overlap * 100, 2)) + "%"
                ...
                classify = "优化"
                root_cause = "接入小区prb资源受限，上行prb利用率" + str(src_ul_prb_ratio) + "%，下行prb利用率" + str(
                    src_dl_prb_ratio) + "%"
                action = "建议通过覆盖控制实现区域负荷分担，必要时载波扩容"
            else:
                classify = "-"
            ...
            if connesttype_list[str(connesttype)] == "RRCSetup":
                # 问题描述
                if servicetype_list[str(servicetype)] == "Voice" or servicetype_list[str(servicetype)] == "IMSData":
                    description = "RRC异常释放，语音业务掉话，携带原因值为" + endcause_list[
                        str(endcause)]
                else:
                    description = "RRC异常释放，数据业务掉线，携带原因值为" \
                                  + endcause_list[str(endcause)]
            elif connesttype_list[str(connesttype)] == "RRCReest":
                if servicetype_list[str(servicetype)] == "Voice" or servicetype_list[str(servicetype)] == "IMSData":
                    description = "RRC重建后，连接异常释放，语音业务掉话，携带原因值为" + endcause_list[
                        str(endcause)]
                else:
                    description = "RRC重建后，连接异常释放，数据业务掉线，携带原因值为" \
                                  + endcause_list[str(endcause)]
            else:
                if servicetype_list[str(servicetype)] == "Voice" or servicetype_list[str(servicetype)] == "IMSData":
                    description = "UE切入成功，业务建立后RRC异常释放，语音业务掉话，携带原因值为" + endcause_list[
                        str(endcause)]
                else:
                    description = "UE切入成功，业务建立后RRC异常释放，数据业务掉线，携带原因值为" \
                                  + endcause_list[str(endcause)]
            # 问题诊断
            if (not pd.isnull(src_alarm_num) and src_alarm_num > 0) or (
                ...
                if (not pd.isnull(src_plansite_flag) and src_plansite_flag == 1) or \
                    ...
                    action = "建议通过参数优化提升电梯口处切换速度"
                else:
                    classify = "优化"
                    root_cause = "用户所在位置覆盖差,信息不足无法进一步定位"
                    action = "建议路测定位弱覆盖位置并整改，提升小区覆盖能力"
            elif not pd.isnull(overshootingsample) and overshootingsample > 0 and not pd.isnull(
                ...
                action = "建议压低服务小区天线下倾角或者降低站高"
            elif not pd.isnull(src_idle_prb_noise_poser) and src_idle_prb_noise_poser > -95:
                classify = "优化"
                root_cause = "服务小区存在上行外部强干扰,上行闲时每PRB噪声功率为" + str(round(src_idle_prb_noise_poser,2)) + "dBm"
                action = "建议排查外部强上行干扰"
            elif not pd.isnull(src_ul_sinr_avg) and src_ul_sinr_avg < -5:
                classify = "优化"
                ...
                classify = "维护"
                root_cause = "切换目标小区存在上行强干扰，上行闲时每PRB噪声功率为" + str(round(tar_idle_prb_noise_poser,2)) + "dBm"
                action = "建议排查目标小区上行外部干扰"
            elif (not pd.isnull(overlap) and overlap > 0.3) or (not pd.isnull(overlap) and tar_overlap_ratio > 0.3):
                classify = "优化"
                root_cause = "问题区域重叠覆盖率高，为" + str(round(overlap * 100, 2)) + "%"
...
## 郑州版本不需要调用
def date_modify(table_name, delta_ms, expert_db_account):
    conn = psycopg2.connect(database=expert_db_account["database"], user=expert_db_account["user"],
                            password=expert_db_account["password"], host=expert_db_account["host"],
                            port=expert_db_account["port"])
    cur = conn.cursor()
    # 时间戳修改
    sql = "update " + table_name + " set endtime =endtime+" + str(
        delta_ms) + ";" + "update " + table_name + " set starttime =starttime+" + str(delta_ms) + ";"
...
if __name__ == '__main__':
    day_num = ${delay_days}
    yestoday, yestoday_date_,year,delta_ms = get_date(day_num)
    #yestoday='20221113'
    ## 北京数据库账密
    # trace_db_account = {"database": "sk_data", "user": "sk", "password": "YJY_tra#tra502",
    ...
    # 获取pm 告警数据
    pm_data, alarm_data, plansite_data, top1_site_data, siteinfo_data = get_analysis_data(yestoday, yestoday_date_,year,expert_db_account)
    ## 建表
    expert_create_talbe(yestoday, expert_db_account)
    ## 数据查询
    trace_data = get_call_recorder(yestoday, trace_db_account)

    if len(trace_data)!=0:
        data_association = form_association(trace_data, pm_data, alarm_data, plansite_data, top1_site_data,
                                            siteinfo_data)
        ## 进程数
        seednum = 20
        tablename = 'lte_abnormal_call_detail_' + str(yestoday)

        if len(data_association) != 0:
            dignosis_output = multi_process(data_association, seednum)
            ## 数据
            data_load(dignosis_output, tablename,expert_db_account)
            #data_upload_tosql(dignosis_output,tablename,"cnio")
            #date_modify("cnio."+tablename, delta_ms,expert_db_account)
            print("异常呼叫话单入库完成")
        else:
            print("异常话单获取失败")
    else:
        print("未查询到trace数据，数据为空")
